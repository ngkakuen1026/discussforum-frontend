...
import DeleteUserFollowerPopup from "./DeleteUserFollowerPopup";

interface UserFollowerListActionDropdownProps {
  follower: UserFollowerType;
  userId: string;
  username: string;
}

const UserFollowerListActionDropdown = ({
  follower,
  userId,
  username,
}: UserFollowerListActionDropdownProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [showDeleteFollowerPopup, setShowDeleteFollowerPopup] = useState(false);
  ...
  return (
    <div className="relative">
        ...
        <ClickOutside onClickOutside={() => setIsOpen(false)}>
          ...
          >
            <Link
              ...
              params={{ userId: follower.follower_user_id.toString() }}
              search={{ page: 1 }}
            ...
            <button
              onClick={() => setShowDeleteFollowerPopup(true)}
      ...
      {showDeleteFollowerPopup && (
        <DeleteUserFollowerPopup
          follower={follower}
          userId={userId}
          username={username}
          onClose={() => setShowDeleteFollowerPopup(false)}
        />
      )}
    </div>
  );
...
export default UserFollowerListActionDropdown;
