...
const AdminBannerPopup = ({ onClose, user }: BannerPopupProps) => {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      await authAxios.post(
        `${adminAPI.url}/users/user/profile/${user?.id}/profile-banner`,
        formData,
        {
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["admin-user-profile", user?.id.toString()],
      });
      onClose();
    },
    onError: () => {
          ...
          <div className="p-8 bg-gray-800/30">
            <p className="text-lg text-gray-400 mb-4 text-center">
              {previewUrl
                ...
                : user?.profile_banner
                  ? "Current Banner"
                  : `No Banner (${user?.username} are using default banner provided by ChatterNest)`}
            </p>
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 rounded-xl text-white font-medium transition disabled:opacity-50 cursor-pointer"
                >
...
export default AdminBannerPopup;
