...
import UserFollowingAction from "./UserFollowingAction";
...
const UserFollowing = () => {
  ...
  const { data: userFollowingsData, isLoading } = useQuery({
    queryKey: [
      "admin-user-followings",
      userId,
      search,
      sort,
      currentPage,
    ...
    queryFn: async () => {
      ...
      const res = await authAxios.get(
        `${adminAPI.url}/user-following/followings/${userId}/search?${params.toString()}`,
      );
      return res.data;
    },
    staleTime: 5 * 60 * 1000,
  ...
  const userFollowings = userFollowingsData?.userFollowingList || [];
  const username = userData?.user?.username || `User${userId}`;
  const pagination = userFollowingsData?.pagination || null;
