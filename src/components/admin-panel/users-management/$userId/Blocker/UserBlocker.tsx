...
import UserBlockerAction from "./UserBlockerAction";
import UserBlockerList from "./UserBlockerList";
...
const UserBlocker = () => {
  ...
  const { data: userBlockerData, isLoading } = useQuery({
    queryKey: [
      "admin-user-blockers",
      userId,
      search,
  ...
  const userBlockers = userBlockerData?.userBlockerList || [];
  const username = userData?.user?.username || `User${userId}`;
  const pagination = userBlockerData?.pagination || null;
