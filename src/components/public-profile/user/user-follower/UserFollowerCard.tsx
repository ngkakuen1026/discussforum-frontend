...
interface UserFollowerPostCardProps {
  followerUser: UserFollowerType;
  removeMutation: UseMutationResult<unknown, Error, number>;
}

const UserFollowerPostCard = ({
  followerUser,
  removeMutation,
}: UserFollowerPostCardProps) => {
  const navigate = useNavigate();

  const handleUnfollow = (e: React.MouseEvent) => {
    e.stopPropagation();
    removeMutation.mutate(followerUser.follower_user_id);
  };
    ...
    <div
      onClick={() =>
        navigate({
          ...
          params: { userId: followerUser.follower_user_id.toString() },
          replace: true,
        })
      }
    ...
    >
      ...
      <div className="p-6 flex flex-col sm:flex-row gap-6 items-center sm:items-start">
        ...
        <div className="shrink-0 flex flex-col items-center justify-between">
          <img
            src={getUserAvatar({
              author_profile_image: followerUser.follower_user_profile_image,
              author_gender: followerUser.follower_user_gender,
            })}
            alt={followerUser.follower_user_username}
          ...
          <UserRoleTag
            user={{
              is_admin: followerUser.follower_user_is_admin,
              gender: followerUser.follower_user_gender,
            }}
          />
        </div>
        ...
        <div className="flex-1 text-center sm:text-left">
            ...
            <div className="flex items-center gap-2 justify-center sm:justify-start">
              <p
                className={`text-xl font-bold ${getUsernameColor({
                  author_gender: followerUser.follower_user_gender,
                  author_is_admin: followerUser.follower_user_is_admin,
                })}`}
              >
                {followerUser.follower_user_username}
              </p>
            </div>
          ...
          <p className="text-gray-300 text-sm mt-2">
            ...
            {formatUserRegistrationDate(
              followerUser.follower_user_registration_date
            )}
          </p>
        </div>
      </div>
...
export default UserFollowerPostCard;
