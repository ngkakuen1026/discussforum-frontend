...
interface UserFollowerToolbarProps {
  followerUsers: UserFollowerType[];
  refreshMyFollower: () => void;
}

const UserFollowerToolbar = ({
  followerUsers,
  refreshMyFollower,
}: UserFollowerToolbarProps) => {
  return (
    <div className="">
      <div className="flex items-center justify-between gap-2">
        <h1 className="text-white text-2xl font-bold">
          My Follower ({followerUsers.length})
        </h1>

        <div className="flex gap-2 ">
          <button
            onClick={refreshMyFollower}
      ...
      <div>
        <h2 className={`text-gray-400 mt-2`}>The user follow you</h2>
      </div>
    </div>
  );
};

export default UserFollowerToolbar;
