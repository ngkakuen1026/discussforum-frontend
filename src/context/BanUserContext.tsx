...
interface UserBanContextType {
  banUser: (userId: number, durationHours: number, reason?: string) => void;
...
export const UserBanProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const value: UserBanContextType = {
    banUser: (userId, durationHours, reason) => 
      banMutation.mutate({ userId, durationHours, reason }),
  ...
  return (
    <UserBanContext.Provider value={value}>
      {children}
